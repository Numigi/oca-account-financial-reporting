...
class ActivityStatement(models.AbstractModel):
    ...
    def _display_activity_lines_sql_q1(self, partners, date_start, date_end,
                                       account_type):
        excluded_accounts_ids = tuple(
            self.env.context.get("excluded_accounts_ids", [])) or (-1,)
        show_only_overdue = self.env.context.get("show_only_overdue", False)
        payment_ref = _("Payment")

        # Exclude currency exchange journal
        excluded_journal_id = self.env.company.currency_exchange_journal_id.id
        journal_clause = ""
        if excluded_journal_id:
            journal_clause = f"AND l.journal_id != {excluded_journal_id}"
        ...
        query = f"""
               ...
               WHERE l.partner_id IN %(partners)s
                   ...
                   AND m.state IN ('posted')
                   {journal_clause} -- <-- CLAUSE AJOUTÉE ICI
                   AND CASE
